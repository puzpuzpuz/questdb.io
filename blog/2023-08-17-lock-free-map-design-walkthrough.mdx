...
discuss this in more detail later. Let’s explore the first issue.
...
1. `get()` still returns a pointer, but the map guarantees the memory behind it
   stays immutable until the caller notifies the map that it’s done and that it
   won’t use the pointer anymore.
...
Ideally, the buffer would be allocated on stack, but this is Java so that’s not
...
no readers. Otherwise, we would be leaking memory. That’s a complication, but it
...
mutation. We said that mutations are rare, so perhaps that’s not a big deal?
Maybe it’s not, but one of the QuestDB design principles is to be conservative
...
Once a map is published for readers, it’s guaranteed to be immutable as long as
...
1. There is a skeleton of `InternalMap`. It does not show any map-related logic,
   ...
   counter.
1. For the first time we see an implementation `getMapForWriters()`. It’s really
...
they won’t access the returned pointer anymore.
...
Why this complexity? Why do we need the retry mechanism at all? It’s to protect
