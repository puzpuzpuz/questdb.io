...
To give you an impression of that fuzzer, let's dive into our ILP protocol [implementation](https://questdb.io/docs/reference/api/ilp/overview/). Creating a single-row table with ILP is as simple as sending the following to `<questdb_host>:9009`:
...
Here we have a [partitioned](https://questdb.io/docs/concept/designated-timestamp/) table named `weather`. The table structure, column names and row values are fully defined in our ILP message. Now, let's send another message over TCP:
