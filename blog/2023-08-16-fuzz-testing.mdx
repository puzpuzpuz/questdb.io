...
As we already mentioned, our first fuzz test was written almost two years ago. It tested InfluxDB Line Protocol protocol by sending semi-random, potentially invalid messages. The first test immediately revealed a number of critical issues, including a few segfaults.

To give you an impression of that fuzzer, let's dive into our InfluxDB Line Protocol protocol [implementation](https://questdb.io/docs/reference/api/ilp/overview/). Creating a single-row table is as simple as sending the following to `<questdb_host>:9009`:
...
Adding new columns, reordering columns, and changing column names are not the only things that we can potentially do over the InfluxDB Line Protocol protocol. 
