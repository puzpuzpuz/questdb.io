...
IPv4 addresses are a common data type with their own unique numerical convention. We often hear from teams who are tracking the activity of users, servers or sensors, that managing IP addresses without native support for subnets and netmasks creates additional complexity. 
...
Yes! IPv4 addresses are stored as strings. 

String arguments may be appear as: 

* an ip address without netmask: `23.62.200.1`
* an ip address with netmask: `5.43.11.8/2` 
* a subnet with netmask: `2.232/16` 

All together, we support the full range of IPv4 addresses.

### Which SQL operations can be performed on IPv4 addresses?

The following operations are supported:

* `<<`: is an ip address strictly contained by subnet? (Boolean)
* `<<=`: is an ip address contained by or equal to subnet? (Boolean)
* `&`: computes bitwise AND (String)
* `~`: computes bitwise NOT (String)
* `|`: computes bitwise OR (String)
* `+`: adds an offset to an ip address (String)
* `-`: ipv4 & int: subtracts an offset from an ip address (String)
* `-`: ipv4 & ipc4: computes the difference between two ip addresses (Long)

There are also two additional functions for testing:
...
- [QuestDB 7.3 release notes](https://github.com/questdb/questdb/releases/tag/7.3)
