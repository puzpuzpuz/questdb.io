...
article ['What is Time-Series Data'](/glossary/time-series-database/)). As a
result, we have seen the emergence of new time-series databases, either
purposefully built to handle time-series data, or as an additional functionality
on top of a general-purpose database. Time series data is often processed with
dedicated tooling. This is because handling this type of data has a few
...
supports Time-Series Collections, which are behind-the-scenes
[materialized views](/glossary/materialized-view/) backed by an internal
collection.
